...
Array Value1,Value2""",
  ...
  SOSCF
    start 0.002
            ...
            st.markdown("""
            `%MaxCore` sets the memory limit **per core** in MB. Use ~75–80% of physical memory.
            """)
...
! BP86 DEF2-SVP""",
    ...
    # Run type selection, including frequency runs for vibrational spectroscopy
    with col3:
        run_type_label = st.selectbox(
            "Run type",
            [
                "Opt + Freq (geometry + IR/Raman)",
                "Freq only (IR/Raman)",
                "Opt (geometry only)",
                "Energy (single point)",
                "EnGrad (energy + gradient)",
            ],
            help="RunTyp selection. Freq/Opt Freq are required for IR/Raman and NEARIR."
        )

    # Spectroscopy-specific toggles (only affect simple XYZ mode)
    include_nearir = st.checkbox(
        "Include NEARIR overtones/combination bands (NEARIR)",
        value=False,
        help="Adds the NEARIR keyword. Requires a frequency calculation (Freq / Opt Freq / NumFreq)."
    )
    include_raman = st.checkbox(
        "Compute Raman intensities (adds %elprop Polar 1)",
        value=False,
        help="Analytical Raman requires an SCF method (HF/DFT) combined with a frequency run."
    )
    ...
    run_type_map = {
        "Opt + Freq (geometry + IR/Raman)": "Opt Freq",
        "Freq only (IR/Raman)": "Freq",
        "Opt (geometry only)": "Opt",
    ...
    # Combine run type with optional NEARIR keyword for anharmonic overtones
    calc_keyword = run_type_keyword
    if include_nearir:
        if "freq" not in run_type_keyword.lower():
            st.warning("NEARIR requested but Run type does not include a frequency calculation; ORCA may not compute NEARIR correctly.")
        calc_keyword = f"{calc_keyword} NEARIR"
    ...
    if current_mode == "Upload ORCA input file":
        st.warning("⚠️ Uploaded ORCA input overrides these settings. The calculation uses your file exactly as written.")
    else:
        st.info("💡 **Tip:** These defaults (B3LYP/def2-SVP/Opt) are reasonable for small organic molecules; adjust as needed.")
...
%output
  ...
  Print[ P_Loewdin ]     1  # Löwdin analysis
end
```
"""
            )
...
"""
    ...
    if input_mode == "Simple XYZ (recommended)":
        ...
        if xyz_upload is not None:
            ...
            if xyz_error:
                st.error(f"❌ XYZ upload error: {xyz_error}")
            else:
                st.session_state.geometry_input = parsed_geometry
                st.success("✅ XYZ file loaded into the coordinate field.")
        ...
        base_input = generate_orca_input(
            ...
            basis=basis_keyword,
            calculation_type=calc_keyword,
            charge=int(charge),
            multiplicity=int(multiplicity),
            units=units,
        )

        # Assemble optional control blocks for simple XYZ mode
        prefix_blocks = ""
        if printlevel_keyword:
            prefix_blocks += f"%output\n  PrintLevel {printlevel_keyword}\nend\n\n"
        if include_raman:
            if "freq" not in run_type_keyword.lower():
                st.warning("Raman requested but Run type does not include a frequency calculation; analytical Raman intensities will not be available.")
            prefix_blocks += "%elprop\n  Polar 1\nend\n\n"

        input_content = prefix_blocks + base_input
        ...
        if uploaded_input is not None:
            ...
            if geometry_from_input:
                ...
                st.success("✅ Geometry extracted from uploaded input and added to the XYZ field.")
                st.caption(f"Parsed charge/multiplicity: {charge} / {multiplicity}")
            else:
                st.warning("⚠️ Could not locate an '* xyz' block to auto-fill geometry.")
    ...
    # Optional post-processing with orca_mapspc (spectrum generation)
    with st.expander("Post-processing: orca_mapspc (spectrum files)"):
        generate_mapspc = st.checkbox(
            "Run orca_mapspc to generate spectrum .dat files",
            value=False,
            help="Requires a frequency calculation (Run type including Freq)."
        )
        mapspc_spectra = st.multiselect(
            "Spectrum type(s)",
            ["IR", "Raman"],
            default=["IR"],
            help="IR is always available for frequency runs; Raman requires that Raman intensities be computed."
        )
        col_w, col_x0, col_x1, col_n = st.columns(4)
        with col_w:
            mapspc_w = st.number_input(
                "Linewidth w (cm⁻¹)",
                min_value=0.0,
                value=25.0,
                help="Gaussian FWHM for peak broadening (-w)."
            )
        with col_x0:
            mapspc_x0 = st.number_input(
                "x₀ start (cm⁻¹, 0 = default)",
                min_value=0.0,
                value=0.0,
                help="Start of spectrum (-x0). Leave 0 to let orca_mapspc choose."
            )
        with col_x1:
            mapspc_x1 = st.number_input(
                "x₁ end (cm⁻¹, 0 = default)",
                min_value=0.0,
                value=0.0,
                help="End of spectrum (-x1). Leave 0 to let orca_mapspc choose."
            )
        with col_n:
            mapspc_n = st.number_input(
                "n points (0 = default)",
                min_value=0,
                value=0,
                help="Number of points (-n). Leave 0 to let orca_mapspc choose."
            )
    
    if st.button("🚀 Run Calculation", type="primary", use_container_width=True):
        if not input_content.strip():
            st.error("❌ Please provide ORCA input (either XYZ or an uploaded .inp file)")
            return
        ...
        if input_mode == "Simple XYZ (recommended)":
            ...
            if validation_errors:
                st.error("❌ ORCA input validation failed:")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with st.status("Running ORCA calculation...", expanded=True) as status:
                ...
                # Try to read full ORCA output regardless of success, if a file exists
                full_output = ""
                ...
                if not success:
                    ...
                    # Still show whatever ORCA printed to help debugging
                    if full_output:
                        st.divider()
                        st.subheader("📄 Full ORCA Output (failed job)")
                        st.text_area(
                            "ORCA Output",
                            value=full_output,
                            height=400,
                        ...
                        st.download_button(
                            ...
                            data=full_output,
                            file_name="orca_output_failed.out",
                            mime="text/plain",
                ...
                # Optional orca_mapspc post-processing (within the same temp dir)
                ir_dat_content = ""
                raman_dat_content = ""
                if generate_mapspc and "freq" in calc_keyword.lower():
                    # Common CLI pieces
                    def build_mapspc_args(kind: str):
                        # Use uppercase spectrum type tokens as shown in orca_mapspc usage (IR, RAMAN, ...)
                        kind_token = kind.upper()
                        args = ["orca_mapspc", output_file, kind_token, "-w", str(mapspc_w)]
                        if mapspc_x0 > 0.0:
                            args.extend(["-x0", str(mapspc_x0)])
                        if mapspc_x1 > 0.0:
                            args.extend(["-x1", str(mapspc_x1)])
                        if mapspc_n > 0:
                            args.extend(["-n", str(mapspc_n)])
                        return args

                    try:
                        if "IR" in mapspc_spectra:
                            try:
                                res_ir = subprocess.run(
                                    build_mapspc_args("IR"),
                                    cwd=temp_dir,
                                    capture_output=True,
                                    text=True,
                                )
                                if res_ir.returncode != 0:
                                    msg_ir = res_ir.stderr.strip() or res_ir.stdout.strip() or "(no message)"
                                    st.warning(f"orca_mapspc IR run failed: {msg_ir}")
                                ir_dat_path = output_file + ".ir.dat"
                                if os.path.exists(ir_dat_path):
                                    with open(ir_dat_path, "r") as f:
                                        ir_dat_content = f.read()
                            except FileNotFoundError:
                                st.warning("orca_mapspc not found in PATH; skipping IR spectrum generation.")

                        if "Raman" in mapspc_spectra:
                            try:
                                res_r = subprocess.run(
                                    build_mapspc_args("RAMAN"),
                                    cwd=temp_dir,
                                    capture_output=True,
                                    text=True,
                                )
                                if res_r.returncode != 0:
                                    msg_r = res_r.stderr.strip() or res_r.stdout.strip() or "(no message)"
                                    st.warning(f"orca_mapspc Raman run failed: {msg_r}")
                                raman_dat_path = output_file + ".raman.dat"
                                if os.path.exists(raman_dat_path):
                                    with open(raman_dat_path, "r") as f:
                                        raman_dat_content = f.read()
                            except FileNotFoundError:
                                st.warning("orca_mapspc not found in PATH; skipping Raman spectrum generation.")
                    except Exception as e:
                ...
                try:
                    ...
                    # Copy core ORCA files if present
                    for fname in ["calculation.inp", "calculation.out", "calculation.gbw", "calculation.hess"]:
                        src = temp_path / fname
                        if src.exists():
                            shutil.copy(src, run_dir / fname)

                    # Copy generated spectrum files, if any
                    ir_src = Path(output_file + ".ir.dat")
                    if ir_src.exists():
                        shutil.copy(ir_src, run_dir / ir_src.name)
                    raman_src = Path(output_file + ".raman.dat")
                    if raman_src.exists():
                        shutil.copy(raman_src, run_dir / raman_src.name)

                    st.info(f"Run files saved in: {run_dir}")
                except Exception as e:
                    st.warning(f"Could not persist run files to ./output: {e}")

            # Parse results (energy and optimized geometry); keep full_output for display
            energy, optimized_geometry, _ = parse_orca_output(output_file)
            ...
            if generate_mapspc and ("IR" in mapspc_spectra or "Raman" in mapspc_spectra):
                ...
                if raman_dat_content:
                    st.markdown("**Raman spectrum (.raman.dat)**")
                    with st.expander("View Raman .dat content", expanded=False):
                        st.text_area(
                            "Raman .dat",
                            value=raman_dat_content,
                            height=200,
                            label_visibility="collapsed",
                        )
                    st.download_button(
                        label="📥 Download Raman .dat",
                        data=raman_dat_content,
                        file_name="spectrum_raman.dat",
                        mime="text/plain",
                        use_container_width=True,
                    )
